...
function replaceHint(editor:vscode.TextEditor, line:number, hint:string, withStr:string):boolean {
  ...
  if (textLine.text.indexOf(hint) >= 0) {
    editor.edit((textEdit)=> {
      textEdit.replace(new vscode.Range(
...
function setUploadedImageURL(imageFileName:string, url:string, editor:vscode.TextEditor, hint:string, curPos:vscode.Position) {
  let description
  if (imageFileName.lastIndexOf('.'))
    description = imageFileName.slice(0, imageFileName.lastIndexOf('.'))
  else
