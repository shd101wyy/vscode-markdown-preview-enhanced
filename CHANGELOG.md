...
- **Start Crossnote Server no longer opens a blank page** — the command's build directory (the `crossnote/` folder populated by `gulp copy-files`) did not include crossnote's `server-app/` output, so the served app shell loaded but its script and stylesheet (`/assets/server-app/server-app.js|css`) 404'd and the browser showed an empty page, while `npx crossnote serve` (whose package ships the folder) worked. The gulp copy now ships `server-app/`, and crossnote is pinned to 0.9.38 — the release whose standalone wiki (crossnote#506/#508) embeds the serve app itself as a read-only shell (replacing the old `wiki-app/` copy, which that release no longer ships), brings preview-theme-matching light/dark chrome with a title-bar toggle, in-wiki theme switching, an embedded graph view and backlinks, TiddlyWiki-style remote-image embedding, and the Windows path fixes that made extension-spawned servers render images and link-opened previews correctly. The preview's About submenu also gains a **Crossnote** item (logo + repo link, handled by the new `openCrossnote` message).
...
- **Build standalone Wiki command** — `Markdown Preview Enhanced: Build standalone Wiki (HTML)` packs the folders of the current workspace (or the folder of the loose file, when no workspace is open) into one standalone, read-only HTML file in the spirit of TiddlyWiki: a fuzzy-filtered file list, one pre-rendered note at a time inside a sandboxed iframe, note-to-note links resolving inside the file, external links opening new tabs, task checkboxes disabled — nothing in the file can write anywhere. Notes are rendered by crossnote's HTML export pipeline (CDN-hosted KaTeX/MathJax/mermaid/wavedrom/vega assets; local images/SVGs embedded as data URIs), with the same config resolution as the extension. A save dialog asks where to store the file (default `index.html`), and the result can be opened right away. The preview's context menu gains the same **Export standalone HTML (wiki)** item, which builds for the whole workspace and shares the save dialog. Requires the upcoming crossnote release exporting `buildWiki` — against older pinned crossnote versions the command explains that the dependency needs updating, and everything else is unaffected. Desktop only.
