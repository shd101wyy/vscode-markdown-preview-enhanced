...
### Picked up from crossnote 0.9.40

The extension now pins crossnote 0.9.40.

- **Local fonts and images in `.crossnote/style.less` load in the preview** — a relative `url(...)` in the user stylesheet (`@font-face { src: url('fonts/MyFont.woff2') }`, `background-image: url(./bg.png)`) never resolved: crossnote compiles style.less and inlines it into a `<style>` tag, so the browser resolved those references against the preview document instead of the stylesheet — inside a VS Code webview nothing exists there. Local fonts silently fell back to the default while remote ones (Google Fonts, any `https://` URL) kept working, which made the failure look arbitrary ([#2424](https://github.com/shd101wyy/vscode-markdown-preview-enhanced/issues/2424) reported by @patriknusszer; fixed in [shd101wyy/crossnote#513](https://github.com/shd101wyy/crossnote/pull/513) by @Waynting). References are now resolved against style.less's own directory when it is loaded and rewritten into webview-loadable URLs for the preview (`file://…` in every export format). Absolute URLs, `url(#fragment)` paint servers, paths escaping the stylesheet's directory, and missing files are left untouched, and `_Start Crossnote Server_` keeps its own root-relative `url(/files/…)` references working ([shd101wyy/crossnote#516](https://github.com/shd101wyy/crossnote/pull/516)).
- **Mermaid and wavedrom CLI exports no longer run through a shell ([CVE-2022-45026](https://nvd.nist.gov/vuln/detail/CVE-2022-45026))** — exporting a mermaid diagram to PNG or a wavedrom diagram to SVG spawned the CLI with `shell: true`, handing it an output path built from the diagram's `filename` attribute (untrusted markdown), the notebook's `imageFolderPath`, and the project directory. Only the filename part was sanitized, so a project or image folder whose name contains a shell metacharacter allowed command injection on export. Both CLIs now spawn with every argument passed literally. This also fixes the long-standing plain failure that exporting from any project whose path contains a space produced a broken command ([shd101wyy/crossnote#512](https://github.com/shd101wyy/crossnote/pull/512) by @Waynting; [shd101wyy/crossnote#274](https://github.com/shd101wyy/crossnote/issues/274) reported by @yuriisanin).
- **The note-index home-directory refusal is now enforced by crossnote itself** — completing the entry above: the pinned library now refuses a home-directory notebook root in every refresh entry point (case-insensitively on Windows, where the vscode-uri round-trip lowercases the drive letter), so the refusal no longer depends on the extension's own check, and a single refresh that stats more than 10,000 filesystem entries logs one warning naming the notebook root, so a runaway index walk is diagnosable from a user report alone ([shd101wyy/crossnote#514](https://github.com/shd101wyy/crossnote/pull/514)).
- **Diagrams render with mermaid 12** — the vendored bundle (and the CDN fallback of the offline HTML export) move from mermaid 11.17.2 to 12.0.0. Guard tests pin that the bundle still parses on crossnote's Node 18 runtime floor, uses no browser APIs newer than the oldest supported webview (Electron 25 / Chromium 114), and that the five mermaid API entry points the preview calls (`initialize`, `init`, `run`, `render`, `registerExternalDiagrams`) are all still functions.
...
- **New commands "Open Current Source in Editor" and "Open Current Source in Editor and Close Preview"** ([#2420](https://github.com/shd101wyy/vscode-markdown-preview-enhanced/pull/2420) by @HappyOnigiri) — Open the Markdown file the focused preview is rendering in a text editor, either alongside the preview or replacing it. This is the way back to the source after following links inside the preview, where the preview shows `subdir/c.md` but the tab it was opened from still refers to `a.md`; it also gives `"previewMode": "Previews Only"` an exit into the editor, since the preview occupies the tab there. The editor opens in the preview's own editor group, and the preview tab is only closed once the editor has opened, so a failure to open leaves the preview in place.

- **Build standalone Wiki command** — `Markdown Preview Enhanced: Build standalone Wiki (HTML)` packs the folders of the current workspace (or the folder of the loose file, when no workspace is open) into one standalone, read-only HTML file, with the same config resolution as the extension. A save dialog asks where to store the file (default `index.html`) and the result can be opened right away; the preview context menu's **Export ▸ Wiki** item builds the same file for the whole workspace and shares the dialog. What the exported file actually is — since crossnote 0.9.37 a frozen, read-only copy of the whole serve app — is described in the crossnote section below. Desktop only.

- **Start/Stop crossnote Server commands** — `Markdown Preview Enhanced: Start Crossnote Server` runs the `crossnote serve` CLI as a child process for the folders of the current workspace (multi-root workspaces serve every folder, exactly like the CLI), loading config the same way the extension does, and offers the server URL in an "Open in browser" notification. `Markdown Preview Enhanced: Stop Crossnote Server` stops it. The port defaults to 3000 (auto-increments when busy) and is configurable via `markdown-preview-enhanced.crossnoteServePort`. The server runs in its own process because crossnote's serve server mutates process-global state (build directory, the file-URL mapper) that the extension host's own previews rely on. The CLI is bundled at build time from the `crossnote/cli` export; a build against a crossnote without it skips the bundle and the commands explain at runtime. Desktop only — the web extension cannot spawn processes.

### Bug fixes

- **The wiki export progress notification closes when the file is saved** — `Build standalone Wiki` showed "Building the standalone wiki…" with a progress notification that stayed open after the "Standalone wiki saved (N notes)" message appeared, until the user dismissed that message: the progress task was awaiting the _saved_ notification's promise, so the progress legitimately considered itself still running until then. The task now ends when the file is written, and the saved message (with its localized **Open** button) is shown after it.

- **Start Crossnote Server no longer opens a blank page** — the command's build directory (the `crossnote/` folder populated by `gulp copy-files`) did not include crossnote's `server-app/` output, so the served app shell loaded but its script and stylesheet (`/assets/server-app/server-app.js|css`) 404'd and the browser showed an empty page, while `npx crossnote serve` (whose package ships the folder) worked. The gulp copy now ships `server-app/`, and crossnote is pinned to 0.9.39 — everything that pin brings is listed in the crossnote sections below.

- **No more "Markdown: Markdown Preview Enhanced: …" double prefix in the command palette** — ten commands (open preview ×3, toggle preview lock, copy source ×3, start/stop crossnote server, build standalone wiki) still carried a leftover `category: "Markdown"` from the 2018-era manifest where titles had no extension prefix; now that every title already reads "Markdown Preview Enhanced: …", VS Code rendered those ten as "Markdown: Markdown Preview Enhanced: …" while the other twenty-four showed without a prefix. The stray category is removed — including from the two new Open Current Source commands of #2420, which arrived with it — and the palette list is uniform. Verified live on macOS with the latest stable VS Code.

- **`cmd`/`ctrl+F` now searches the preview in Previews-Only mode** — the regular preview panel enables VS Code's webview find widget, but the custom-editor preview (used when `previewMode` is _Previews Only_, or via _Reopen Editor With… → Markdown Preview Enhanced_) never did, so pressing `cmd`/`ctrl+F` with the preview focused silently did nothing there ([#2412](https://github.com/shd101wyy/vscode-markdown-preview-enhanced/issues/2412) reported by @zhangdengjie). The custom editor now registers with `enableFindWidget: true`, giving it the same find bar (match highlighting, `enter`/`shift-enter` navigation, scroll markers) as the panel preview. Verified on macOS with the latest stable VS Code.
- **No more "notes are not indexed" notification spam when no folder is open** — the filesystem-root warning introduced with #2376 fired whenever the resolved notebook root was a filesystem root, including the no-workspace fallback root (the standalone file's own directory): opening a markdown file directly under a drive root (e.g. `d:\notes.md` with no folder open) nagged with a notification on every markdown editor activation, even for users who never opened a preview or asked for note indexing ([#2413](https://github.com/shd101wyy/vscode-markdown-preview-enhanced/issues/2413) reported by @ArtemGr). The warning now only fires when the root is an actual workspace folder — i.e. the user deliberately opened a drive root as their workspace — and remains once per root per session there. Note indexing is still refused for filesystem roots either way; only the notification is skipped, so #2376's whole-machine walk protection is unchanged.

### Picked up from crossnote 0.9.39

The extension now pins crossnote 0.9.39 (this PR originally pinned 0.9.38; the sections below still describe those releases).

- **All export formats work in `crossnote serve`** — the preview's Export submenu in the serve app (opened by _Start Crossnote Server_) offered only **Wiki**; it now carries the full set the VS Code extension has: HTML (offline / CDN-hosted), Chrome (Puppeteer) PDF/PNG/JPEG, Prince PDF, ePub/Mobi/PDF/HTML eBooks, Pandoc, and Save as Markdown. The serve server runs the same engine exporters the extension host does, writes the output next to the source file, and reports the destination — or the failure — as a toast in the app (presentation-mode Prince returns its print-it-yourself link, as in the extension). Chrome auto-detects when `chromePath` is empty, and a failed export no longer can kill the server: a third-party library leaking a promise rejection (puppeteer's launch-failure cleanup on Windows does exactly that) is logged while the server keeps serving. The read-only standalone wiki keeps the submenu hidden — it has no host process.
- **Zen mode toggles the preview, not the shell** — the context menu's "Zen Mode" item in the serve app and the standalone wiki hid the app shell's title bar while leaving the preview untouched: the preview's `enablePreviewZenMode` config was never flipped, so the item always left the preview in zen mode and could never turn it off — which also made "Open In-preview Editor" (hidden while zen is on) unusable. The item now flips `enablePreviewZenMode` exactly like the extension's setting toggle: the serve server persists the flipped value and reloads every preview with it, and a wiki file stores the override in `localStorage` (keyed per wiki, defaulting to the build-time value, which now ships in the payload). The shell-level zen mode — hidden title bar, "Exit zen mode" button, Esc-forwarding, and the welcome card's "Esc — exit zen mode" hint — is removed; Esc inside a preview keeps toggling the outline.
- **`--vscode` serve no longer corrupts `settings.json` with a trailing `//` comment** — inserting a new `markdown-preview-enhanced.*` key appended its separator comma directly after the last entry, but when that entry is a commented-out setting the comma lands _inside_ the comment: the new key ends up without a separator and the whole file stops parsing. The damage cascaded silently — the VS Code settings layer failed to load (the serve preview fell back to the default light theme and the zen toggle appeared dead) and every later theme/zen change refused to edit the "unparseable" file, so switching themes stopped working entirely. The writer now tries each separator placement and keeps the first whose result still parses (never writing a file the reader cannot load), and a failed persist surfaces as an error toast in the app instead of only in the server console.
- **The preview notice stays readable** — the transient alert (e.g. "The in-preview editor is unavailable in Zen Mode…") auto-dismissed after 4 s at the bottom of the preview; it now renders at the top, keeps itself on screen for 10 s, and gains a close button (localized in all eleven languages).
- **`About ▸ Crossnote` shows the logo on the right** — the logo moved from the left of the label to after it (20px, matching the menu's other icons), lining up with the trailing 😊 of "Sponsor This Project 😊".
- **crossnote's API docs on GitHub Pages deploy again** — the Pages workflow only triggered on pushes to crossnote's dormant `master` branch, so https://shd101wyy.github.io/crossnote/ had been stuck at 0.9.31 since June; it now deploys on every release (pushes to `develop`), and crossnote's README heading no longer says `[WIP]`.

### Picked up from crossnote 0.9.36–0.9.38

The user-visible changes of everything crossnote shipped after the bundled 0.9.35 follow, grouped per crossnote release (the two 0.9.36 CLI mega-features are the engines behind the extension commands described above).

**crossnote 0.9.36** (the `crossnote serve` and `crossnote build-wiki` CLIs — driven by the extension's Start Crossnote Server and Build standalone Wiki commands, including the serve server's loopback hardening: `Host`/`Origin` gating so websites cannot drive `/api/command`, and `CSP: sandbox` on untrusted workspace documents):

- **`notebook.trustedScriptRoots` — head.html scripts can load from host-named directories** — The 0.9.35 opt-in (`notebook.previewScriptsEnabled`) re-enabled `<script src>` tags in `.crossnote/head.html`, but only for files inside the notebook directory, so the _global_ config directory's head.html (whose whole point is behaviour the user wants in every preview) could not load the scripts sitting next to it — those files live outside every workspace. Host applications can now set `Notebook.trustedScriptRoots: string[]` (absolute paths, default `[]`) to name extra roots — e.g. the `markdown-preview-enhanced.configPath` directory, taken from its user-scope value only, so a repository's `.vscode/settings.json` cannot redirect the "global" directory at itself. When opted in, a relative `src` that doesn't resolve inside the notebook directory is tried against each trusted root; the first candidate that is an existing file and contained (realpath, symlinks included) in the notebook directory or a root is kept and rewritten to a file URL. Inline scripts, URL-scheme sources, the `@import "*.js"` channel (still notebook-directory-only — it is untrusted note content, unlike the host-passed head.html), and the script-stripping export paths are unchanged. Like `previewScriptsEnabled`, the field is deliberately not part of `NotebookConfig`: repository files can never name a trusted root ([#446](https://github.com/shd101wyy/crossnote/issues/446), proposal by @brian-mann).
- **Fix script smuggling through body-leading head.html content** — `resolvePathsInHeader` stripped `<script>` tags from the parsed head.html tree but returned `$('head').html() || header`: when the fragment's content landed in `<body>` instead of `<head>` (any leading text, `<div>`, `<p>`, … — the parser routes body elements there), the stripped head came back as the empty string, the `||` fell back to the _raw_ header, and the very scripts that had just been removed were shipped into the preview `<head>` unstripped — partially defeating the [0.9.31](#0931---2026-06-08) hardening (GHSA-mcwg-4j78-qwv3) for such head.html files. The fallback now only triggers when there is no `<head>` at all; an emptied head correctly returns `''`. Found while testing the trusted-script-roots feature above, whose script-only global head.html hits the same empty-head path whenever a tag is removed.
- **Code text is selectable again in blocks using `{highlight=...}`** — the band wrapper is an absolutely positioned overlay whose box covers the full width of every line preceding a band, and its only content is the invisible newlines that push the band down. Mousedown landed on the wrapper instead of the code, so dragging selected its newlines and no code text — in the lines above a highlight nothing could be selected at all ([vscode-mpe#2409](https://github.com/shd101wyy/vscode-markdown-preview-enhanced/issues/2409) reported by @chz-cn). `pointer-events: none` now lives on the wrapper (inherited by the band, which previously declared it redundantly), so the whole overlay stays out of hit-testing in every theme and export path.
- **`{highlight=...}` bands no longer paint over the code text** — the absolutely positioned highlight bands rendered above the code, so with the github-dark code theme (opaque band) the highlighted lines were completely hidden, and with the vscode code theme (translucent band) the token colors were washed out by the overlay ([vscode-mpe#2403](https://github.com/shd101wyy/vscode-markdown-preview-enhanced/issues/2403), [vscode-mpe#2404](https://github.com/shd101wyy/vscode-markdown-preview-enhanced/issues/2404), follow-ups to #2378, reported by @chz-cn). The shared `style-template.css` now raises the `code` element — and the line-number gutter — above the bands, turning them into an underlay in every theme and export path; github-dark's dead `pre[class*='language-'] > code[class*='language-']` guard (its `code` never carries a `language-` class, so the selector never matched crossnote markup) is removed.

**crossnote 0.9.37**:

- **The standalone wiki is now a read-only `crossnote serve`** — `crossnote build-wiki` no longer packs the notes into a custom shell with its own file-list sidebar: the single file now embeds the _serve app itself_ (titlebar, tabs, split panes, `Ctrl/Cmd+P` fuzzy picker, zen mode) and every note as a regular preview page — the exact page the serve server renders at `/preview?file=…` — shown in sandboxed (`allow-scripts`) iframes. The shell replays each note's `updateHtml` payload when its frame finishes loading, the same message the serve server broadcasts, so previews initialize identically (live-bound links, TOC sidebar, diagrams, math). Shared assets (preview.js, mermaid, KaTeX/MathJax, themes, `@import`ed workspace js/css) are stored once and referenced by `crossnote-wiki-asset:<id>` tokens that the shell inlines when a note is opened; stylesheet `url(…)` references (KaTeX and Font Awesome fonts) are rewritten to data URIs at build time so inlined styles keep working without a base URL; local images travel as data URIs in both the first-paint `data-html` and the update payload. Note-to-note links resolve against the embedded payload (opening a tab), non-note targets get an explanatory toast, and external links open new browser tabs. Everything writable is hidden or inert via the new `WebviewConfig.isWiki` flag (`wiki-readonly` body class): the Edit Markdown submenu, the in-preview editor, code-chunk run buttons, task-list checkbox toggles and every export — the file is read-only by construction. The old `src/wiki-app` client is gone. `MarkdownEngine.generateHTMLTemplateForPreview` accepts an optional `parsedOutput` so callers that need the parse result anyway (the wiki builder, which also ships it as the update payload) don't render every note twice.
- **`Export ▸ Wiki` replaces the top-level "Export standalone HTML (wiki)" context-menu item** — the wiki export moved under the existing Export submenu and is now called simply **Wiki** (like **Pandoc** next to it). The Export submenu is also shown in the standalone serve app — where it contains just the Wiki item, since the other exports need a VS Code host — and is hidden entirely in the read-only wiki.
- **Inter-note links work again in `crossnote serve`** — the serve server rewrites local link hrefs to its `/files/…` mount, but the browser app resolved clicked hrefs as if they were bare root-relative paths, producing `<root>/files/notes/other.md` — a path that doesn't exist — so clicking a wikilink or a markdown link to another note opened a "Failed to render" tab. The app now undoes the mount prefix (honoring the `?root=` hint the mapper adds for multi-root setups) before resolving, so clicked links open the actual target note.
...
- **Light/dark shell theme matching the preview theme** — the serve app and the standalone wiki chrome (title bar, tabs, panes, welcome card, file picker) was dark-only; it now follows the tone of the first workspace's preview theme (light themes like `github-light.css` → light chrome, dark themes → dark chrome; previews themselves keep their own per-workspace themes, and multi-root setups follow the first root's config — the one that anchors the shell). Themes without an opinion (`none.css`, unknown names) follow the OS color scheme. A sun/moon toggle on the title bar flips the chrome and persists the choice in `localStorage`, overriding the theme-derived mode until cleared. In serve, switching the preview theme from the context menu re-tones the chrome live; in the wiki, the chrome follows the stored theme selection.
- **Graph view opens as a pane, and works in the standalone wiki** — the preview footer's graph view button no longer opens a separate browser tab: the graph view now opens beside the active pane (like VS Code's `ViewColumn.Beside`), as a "Graph" tab in the tab strip; clicking it again refreshes its anchor note in place, and exactly one graph tab exists at a time. In serve, the page runs the unmodified graph-view webview bundle behind an injected shim fed by the new `/api/graph` endpoint (first call walks the vault, later ones hit the note cache), and the backlinks toggle computes real backlinks server-side from the note index, delivering them into the preview over SSE — no more eternal loading spinner. The wiki supports both too: the graph data and the graph-view bundle are embedded in the file, the pane's sandboxed frame reads them through a shim, node clicks resolve back into the wiki by note key, and the shell answers `showBacklinks` directly from the embedded payload.
- **Backlinks work in the standalone wiki** — every note's backlinks are computed at export time and embedded in the payload (paths scrubbed to note keys; the reference snippets' link hrefs rewritten to `file:///<key>` so they navigate inside the wiki), and the shell answers the preview's `showBacklinks` request directly — same message the extension and the serve server send. The footer's "Toggle backlinks" and "Open graph view" buttons are no longer hidden in the wiki.
- **Theme switching works in the standalone wiki** — the preview's right-click menu (Preview Theme / Code Block Theme / Reveal.js Theme) now switches themes in a wiki file, persisting the choice in `localStorage` (keyed per wiki) and re-rendering the open tabs with it — same reload semantics as the serve server's theme change. Every available stylesheet (~300 KB) ships in the file once: the three theme slots use stable `crossnote-wiki-theme:<slot>` tokens that the shell resolves against the stored selection when assembling a note, `auto.css` code-block themes follow the (possibly just switched) preview theme via the embedded resolution map, and the page's config meta is rewritten so the menu marks the active theme. Unknown stored names (an older build of the file, or a different wiki sharing the origin's storage) fall back to the build-time themes per slot.
...
- **`Esc` exits zen mode from inside previews** — the welcome card advertised "Esc — exit zen mode", but with focus inside a preview (the common case) Esc toggled the sidebar TOC and never reached the app, so zen mode could only be left by clicking its button. The injected frame shim now learns from the shell whether zen mode is active (`__serverAppZenMode`) and forwards Esc to it in that case — the TOC toggle keeps Esc when zen is off, the promise on the welcome card becomes true, and both the serve server and wiki files get the behavior.
- **The crossnote app uses the real crossnote logo** — the titlebar and welcome card showed a placeholder "c" glyph; both now show the crossnote logo (vendored from [crossnote-app](https://github.com/0xGG/crossnote-app)'s `public/logo.svg`, inlined as a data URI so it travels inside standalone wiki files too).
- **`About ▸ Crossnote` context-menu item** — the preview's About submenu gains a "Crossnote" entry (the logo on its left) opening https://github.com/shd101wyy/crossnote — in the VS Code extension (via the new `openCrossnote` message and host command), the serve server and standalone wiki alike.
- **Previews opened from links showed only "Something is wrong" (serve)** — tabs are keyed by whatever string opened them, and the three producers disagree: the file picker and the watcher hand out native paths (`c:\…\x.md`), link resolution produces posix-style ones (`c:/…/x.md`), and the server echoes `path.resolve`d native paths. The app routed SSE updates (`updateHtml`, `iframeMessage`) and file deletions by exact string match, so an update for `c:\…\ASYNC_AWAIT.md` never reached the tab keyed `c:/…/ASYNC_AWAIT.md` — the preview hung on its loading screen while the same file opened fine from the picker. Routing now compares spellings tolerantly (`sameServeFile`: separator-insensitive always, case-insensitive for windows drive paths, posix stays case-sensitive), and opening a file that is already open under another spelling activates the existing tab instead of duplicating it.
- **Serve previews and wiki exports from the extension showed dead `file:///` image links on Windows** — VS Code hands hosts workspace paths with a lowercase drive letter (`c:\…` from `fsPath`) while the engine normalizes paths to `C:\…` (through vscode-uri), and the serve URL mapper's root-containment check compared case-sensitively — so every workspace file fell through to the `file:///` fallback: preview images 404'd in the browser and wiki exports embedded `file:///` srcs instead of data URIs (the request gate also 400'd URLs whose drive-letter case differed from the spawn). `isPathWithinRoot` now compares case-insensitively on Windows (posix stays case-sensitive; the name-prefix sibling still fails), covering the serve mapper, the `/files` mount resolver, the request gate and the wiki builder's asset/image mapping alike.
...
- **The new wiki/server notifications are localized** — the sixteen user-facing strings added with the Build standalone Wiki and Start/Stop Crossnote Server features (the "Building the standalone wiki…" progress, "Standalone wiki saved (N notes)", the save-dialog title, the server running/stopped/failed messages and their **Open** / **Open in browser** buttons) were English-only; they are now routed through `vscode.l10n.t` with entries in the source bundle and translations for every supported locale, with the message buttons compared against their localized labels so the actions work in every language.
...
The following entries are also taken verbatim from [crossnote 0.9.35](https://github.com/shd101wyy/crossnote/releases/tag/0.9.35).
