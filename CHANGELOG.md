...
- **Restore the vendored `dependencies/mermaid/mermaid.min.js` that an unrelated crossnote PR had committed as a 0-byte file** — crossnote #468 accidentally swept a truncated bundle (a failed `wget -O` empties its output file) into its diff, so every mermaid diagram rendered blank from the offline bundle. The file is restored byte-identical to the official jsDelivr dist of 11.17.2, and a new bundle test guards the size + IIFE signature so an empty or truncated bundle fails CI instead of shipping.
